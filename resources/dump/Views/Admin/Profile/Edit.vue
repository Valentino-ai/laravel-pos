...
const form = reactive({
    name: '',
    email: '',
    current_password: '', // For password confirmation during deletion
...
const deleteAccount = async () => {
    try {
        ...
        if (response.data.success) {
            ...
            router.push('/'); // Redirect to login page after deletion
        } else {
...
<template>
    <div>
        ...
        <div v-if="showConfirmPassword">
            <!-- Password confirmation form for account deletion -->
        ...
        <button v-if="!showConfirmPassword" @click="showConfirmPassword = true" class="bg-red-500 text-white px-6 py-2 rounded-lg">

<template>
    <DashboardLayout>
      <div>
        ...
        <div v-else-if="products.data.length === 0" class="text-center">
          <p>No products available</p>
        </div>
  
        <ul v-else>
          <li v-for="(product, index) in products.data" :key="product.id" class="product-item">
            <strong>{{ index + 1 + (currentPage - 1) * 10 }}. {{ product.name }}</strong>
        ...
        <div v-if="products.total > 0" class="pagination-controls text-center">
